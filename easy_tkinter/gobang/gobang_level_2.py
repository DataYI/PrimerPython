...
class ControlPanel(Frame, ShowCoordsMixIn):
    ...
    def set_widget(self):
        self.button = Button(self, text='重新开始', bg='#CDBA96', state=DISABLED)
        self.button.place(x=200, y=5, width=80, height=30)
...
def events_bind():
    control_panel.button.config(command=restart)
...
def run():
    ...
    if chess_board.is_started:
        control_panel.button.config(state='normal')
    else:
        control_panel.button.config(state='disabled')
